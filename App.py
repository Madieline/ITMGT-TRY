...
def plot_map(title, coords, place_coords, place_labels):
    ...
    ax.plot(*zip(*coords), color='lightgray', label='Route') # plots the route in gray color 
    ax.set_title(title, fontsize=10, pad=20) # title settings
    ax.axis('off') # to only show the graph itself and replicate an image of the route 
    return fig, ax

def highlight_route(ax, start, end, line_coords): # defines the route to be highlighted 
    ...
    ax.plot(highlighted_x, highlighted_y, color='#0305C6', linewidth=1, label=f'Route {start} to {end}') # labels and plots
